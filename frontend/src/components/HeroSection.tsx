...
export default function HeroSection() {
  ...
  return (
    <section
      id="inicio"
      className="bg-[#FFFFFF] px-6 pt-28 pb-16 text-[#0F172A]"
    >
      <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-2">
        {/* Lado izquierdo */}
        <div className="max-w-xl">
          <span className="inline-block rounded-full bg-[#DBEAFE] px-4 py-1 text-sm font-medium text-[#111827]">
          ...
          <p className="mt-5 text-base leading-7 text-[#475569] md:text-lg">
            {activeSlide.description}
          </p>

          <div className="mt-8 flex flex-wrap gap-4">
            <button className="rounded-full bg-[#1D4ED8] px-6 py-3 text-sm font-semibold text-white shadow-md transition hover:bg-[#1E3A8A]">
              Encuentra tu tienda
            </button>

            <button className="rounded-full border border-[#1D4ED8] px-6 py-3 text-sm font-semibold text-[#111827] transition hover:bg-[#1D4ED8] hover:text-white">
              Ver servicios
            </button>
          </div>
  ...
  return (
    <button
      ...
      className={`group relative overflow-hidden rounded-3xl shadow-md transition duration-300 hover:scale-[1.02] ${
        isActive ? "ring-4 ring-[#1D4ED8]/40" : ""
      } ${className}`}
    >
      <img
        src={slide.image}
